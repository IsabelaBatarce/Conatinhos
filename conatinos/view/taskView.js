class TaskView {
  constructor (controller, model) {
    ...
    this.field1 = document.querySelector('#taskfield1');
  ...
  createTask () {
    if (this.field.value == '') {
      ...
      this.controller.createTask(this.field1.value);
      this.show();
    }
    ...
    for (var i = 0; i < this.model.data.length; i++ ) {
      ...
      this.list.innerHTML += '<div class="grid_12 task_container">' +
        ...
        '<button type="button" data-id="'+task.code+'" onclick="view.removeTask('+task.code+')" name="completeButton">Remover</button>'+
          //'<button type="button" data-id="'+task.code+'" onclick="view.modifyTask('+task.code+')" name="completeButton">Modificar</button>'+
        '</div>'+
      '</div>';
    }
  }
